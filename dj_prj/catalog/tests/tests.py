...
class CategoryListViewTest(APITestCase):
    ...
    def test_get_categories_from_cache(self):
        cache.set(
            "category_list", [{"id": self.category.id, "name": self.category.name}]
        )
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data, [{"id": self.category.id, "name": self.category.name}]
        )
...
class ProductListViewTest(APITestCase):
    def setUp(self):
        ...
        self.product = Product.objects.create(
            name="Product 1", category=self.category, price=100.0, stock=10
        )
    ...
    def test_get_products_from_cache(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data, [{"id": self.product.id, "name": self.product.name}]
        )
...
class SupplierListViewTest(APITestCase):
    ...
    def test_get_suppliers_from_cache(self):
        cache.set(
            "supplier_list", [{"id": self.supplier.id, "name": self.supplier.name}]
        )
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data, [{"id": self.supplier.id, "name": self.supplier.name}]
        )
